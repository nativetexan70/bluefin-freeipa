...
### GDM Login Screen — Exclude the Local Admin Account, Not the Whole List

The goal is for GDM's account list to show domain accounts, not this image's one local sudo/wheel fallback account — not to disable the list outright. AccountsService populates its list for local and domain accounts the same way: lazily, as each one actually logs in once (sssd's `enumerate = false` default is unrelated to this and is left alone — domain accounts don't need enumeration to eventually appear here). So the only thing that needs active suppression is the local admin account specifically.
